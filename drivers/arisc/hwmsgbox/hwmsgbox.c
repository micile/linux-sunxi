 ...
 */
	...
	u1_en  =  readl(IO_ADDRESS(AW_MSGBOX_IRQ_EN_REG(AW_HWMSG_QUEUE_USER_AC327)));
	u1_pend =  readl(IO_ADDRESS(AW_MSGBOX_IRQ_STATUS_REG(AW_HWMSG_QUEUE_USER_AC327)));
	if ((u1_en & u1_pend) == 0)
	{
...
//	ARISC_INF("ac327 msgbox interrupt handler...\n");
	...
	while (readl(IO_ADDRESS(AW_MSGBOX_MSG_STATUS_REG(ARISC_HWMSGBOX_ARISC_ASYN_TX_CH)))) {
		...
		if (arisc_message_valid(pmessage)) {
			...
			ARISC_ERR("invalid message received: pmessage = 0x%x. \n", (__u32)pmessage);
		}
	}
	/* clear pending */
...
struct arisc_message *arisc_hwmsgbox_query_message(void)
{
	...
	if (readl(IO_ADDRESS(AW_MSGBOX_MSG_STATUS_REG(ARISC_HWMSGBOX_ARISC_ASYN_TX_CH)))) {
		...
		if (arisc_message_valid(pmessage)) {
			...
			ARISC_ERR("invalid asyn message received: pmessage = 0x%x. \n", (__u32)pmessage);
			return NULL;
		}
		/* clear pending */
		arisc_hwmsgbox_clear_receiver_pending(ARISC_HWMSGBOX_ARISC_ASYN_TX_CH, AW_HWMSG_QUEUE_USER_AC327);
	...
	if (readl(IO_ADDRESS(AW_MSGBOX_MSG_STATUS_REG(ARISC_HWMSGBOX_ARISC_SYN_TX_CH)))) {
		...
		if (arisc_message_valid(pmessage)) {
			...
			ARISC_ERR("invalid syn message received: pmessage = 0x%x. \n", (__u32)pmessage);
			arisc_hwmsgbox_clear_receiver_pending(ARISC_HWMSGBOX_ARISC_SYN_TX_CH, AW_HWMSG_QUEUE_USER_AC327);
			return NULL;
		}
		arisc_hwmsgbox_clear_receiver_pending(ARISC_HWMSGBOX_ARISC_SYN_TX_CH, AW_HWMSG_QUEUE_USER_AC327);
