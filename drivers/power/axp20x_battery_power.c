/*
 * AXP20x PMIC battery power supply status driver
 *
 * Copyright (C) 2015 Hans de Goede <hdegoede@redhat.com>
 * Copyright (C) 2014 Bruno Prémont <bonbons@linux-vserver.org>
 *
 * This program is free software; you can redistribute it and/or modify it
 * under  the terms of the GNU General  Public License as published by the
 * Free Software Foundation;  either version 2 of the License, or (at your
 * option) any later version.
 */

#include <linux/device.h>
#include <linux/init.h>
#include <linux/interrupt.h>
#include <linux/kernel.h>
#include <linux/mfd/axp20x.h>
#include <linux/module.h>
#include <linux/of.h>
#include <linux/platform_device.h>
#include <linux/power_supply.h>
#include <linux/regmap.h>
#include <linux/slab.h>

#define DRVNAME "axp20x-battery-power-supply"

#define AXP20X_PWR_MODE_BATT_PRESENT	BIT(5)
#define AXP20X_PWR_STATUS_BATT_CHARGING	BIT(2)
#define AXP20X_ADC_EN1_BATT_CURR	BIT(6)
#define AXP20X_ADC_EN1_BATT_VOLT	BIT(7)
#define AXP20X_FG_ENABLE   BIT(7)
#define AXP20X_FG_PERCENT  (0x7f)

struct axp20x_battery_power {
	struct device_node *np;
	struct regmap *regmap;
	struct power_supply *supply;
};

static irqreturn_t axp20x_battery_power_irq(int irq, void *devid)
{
	struct axp20x_battery_power *power = devid;

	power_supply_changed(power->supply);

	return IRQ_HANDLED;
}

static int axp20x_battery_power_get_property(struct power_supply *psy,
	enum power_supply_property psp, union power_supply_propval *val)
{
	struct axp20x_battery_power *power = power_supply_get_drvdata(psy);
	int bpercent;
	unsigned int input;
	int ret;

	switch (psp) {
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		ret = axp20x_read_variable_width(power->regmap,
					 AXP20X_BATT_V_H, 12);
		if (ret < 0)
			return ret;

		val->intval = ret * 1100; /* 1 step = 1.1 mV */
		return 0;
	case POWER_SUPPLY_PROP_CAPACITY:
		ret = regmap_read(power->regmap, AXP20X_FG_RES, &bpercent);
		val->intval = bpercent & 0x7F;
		return 0;
	default:
		break;
	}

	/* All the properties below need the power mode reg value */
	ret = regmap_read(power->regmap, AXP20X_PWR_OP_MODE, &input);
	if (ret)
		return ret;

	switch (psp) {	
	case POWER_SUPPLY_PROP_PRESENT:
		val->intval = !!(input & AXP20X_PWR_MODE_BATT_PRESENT);		
		return 0;
	default:
		break;
	}

	/* All the properties below need the input-status reg value */
	ret = regmap_read(power->regmap, AXP20X_PWR_INPUT_STATUS, &input);
	if (ret)
		return ret;

	switch (psp) {	
	case POWER_SUPPLY_PROP_CURRENT_NOW:		
		if (input & AXP20X_PWR_STATUS_BATT_CHARGING) {
			ret = axp20x_read_variable_width(power->regmap,
							 AXP20X_BATT_CHRG_I_H, 12);
			if (ret < 0)
				return ret;
		} else {
			ret = axp20x_read_variable_width(power->regmap,
							 AXP20X_BATT_DISCHRG_I_H, 12);
			if (ret < 0)
				return ret;
			ret = -ret;
		}
		val->intval = ret * 500; /* 1 step = 0.5 mA */
		return 0;
	case POWER_SUPPLY_PROP_STATUS:
		if (input & AXP20X_PWR_STATUS_BATT_CHARGING) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
		} else {
			val->intval = POWER_SUPPLY_STATUS_DISCHARGING;			
		}
		return 0;
		
	default:
		return -EINVAL;
	}

	return 0;
}

static enum power_supply_property axp20x_battery_power_properties[] = {
	POWER_SUPPLY_PROP_PRESENT,
	POWER_SUPPLY_PROP_VOLTAGE_NOW,
	POWER_SUPPLY_PROP_CURRENT_NOW,
	POWER_SUPPLY_PROP_STATUS,
	POWER_SUPPLY_PROP_CAPACITY,
};

static enum power_supply_property axp22x_battery_power_properties[] = {
	POWER_SUPPLY_PROP_PRESENT,
	POWER_SUPPLY_PROP_VOLTAGE_NOW,
	POWER_SUPPLY_PROP_CURRENT_NOW,
	POWER_SUPPLY_PROP_STATUS,
	POWER_SUPPLY_PROP_CAPACITY
};

static const struct power_supply_desc axp20x_battery_power_desc = {
	.name = "axp20x-battery",
	.type = POWER_SUPPLY_TYPE_BATTERY,
	.properties = axp20x_battery_power_properties,
	.num_properties = ARRAY_SIZE(axp20x_battery_power_properties),
	.get_property = axp20x_battery_power_get_property,
};

static const struct power_supply_desc axp22x_battery_power_desc = {
	.name = "axp20x-battery",
	.type = POWER_SUPPLY_TYPE_BATTERY,
	.properties = axp22x_battery_power_properties,
	.num_properties = ARRAY_SIZE(axp22x_battery_power_properties),
	.get_property = axp20x_battery_power_get_property,
};

static int axp20x_battery_power_probe(struct platform_device *pdev)
{
	struct axp20x_dev *axp20x = dev_get_drvdata(pdev->dev.parent);
	struct power_supply_config psy_cfg = {};
	struct axp20x_battery_power *power;
	static const char * const axp20x_irq_names[] = {
		"BATT_PLUGIN", "BATT_REMOVAL", NULL };
	static const char * const axp22x_irq_names[] = {
		"BATT_PLUGIN", "BATT_REMOVAL", NULL };
	static const char * const *irq_names;
	const struct power_supply_desc *battery_power_desc;
	int i, irq, ret;
	u32 ocv[16], rdc;

	if (!of_device_is_available(pdev->dev.of_node))
		return -ENODEV;

	if (!axp20x) {
		dev_err(&pdev->dev, "Parent drvdata not set\n");
		return -EINVAL;
	}

	power = devm_kzalloc(&pdev->dev, sizeof(*power), GFP_KERNEL);
	if (!power)
		return -ENOMEM;

	power->np = pdev->dev.of_node;
	power->regmap = axp20x->regmap;

	if (of_device_is_compatible(power->np,
			"x-powers,axp202-battery-power-supply")) {

		/* Enable acin voltage and current measurement */
		ret = regmap_update_bits(power->regmap, AXP20X_ADC_EN1,
			AXP20X_ADC_EN1_BATT_CURR | AXP20X_ADC_EN1_BATT_VOLT,
			AXP20X_ADC_EN1_BATT_CURR | AXP20X_ADC_EN1_BATT_VOLT);
		if (ret)
			return ret;

		battery_power_desc = &axp20x_battery_power_desc;
		irq_names = axp20x_irq_names;
	} else if (of_device_is_compatible(power->np,
			"x-powers,axp221-battery-power-supply")) {

		/* Enable acin voltage and current measurement */
		ret = regmap_update_bits(power->regmap, AXP20X_ADC_EN1,
			AXP20X_ADC_EN1_BATT_CURR | AXP20X_ADC_EN1_BATT_VOLT,
			AXP20X_ADC_EN1_BATT_CURR | AXP20X_ADC_EN1_BATT_VOLT);
		if (ret)
			return ret;
			
		battery_power_desc = &axp22x_battery_power_desc;
		irq_names = axp22x_irq_names;
	} else {
		dev_err(&pdev->dev, "Unsupported AXP variant: %ld\n",
			axp20x->variant);
		return -EINVAL;
	}

	// ???
	ret = of_property_read_u32_array(power->np, "battery.resistance", &rdc, 1);
	if (ret != 0)
		rdc = 100;

	// Enable the fuel gauge
	regmap_update_bits(axp20x->regmap, AXP20X_FG_RES, AXP20X_FG_ENABLE, 0x00);
	regmap_update_bits(axp20x->regmap, AXP20X_RDC_H, 0x80, 0x00);
	regmap_update_bits(axp20x->regmap, AXP20X_RDC_L, 0xff, (rdc * 10000 + 5371) / 10742);
	regmap_update_bits(axp20x->regmap, AXP20X_RDC_H, 0x1f, ((rdc * 10000 + 5371) / 10742) >> 8);
	if (of_find_property(power->np, "battery.ocv", NULL))
		for (i = 0; i < ARRAY_SIZE(ocv); i++) {
			ret = regmap_update_bits(axp20x->regmap, AXP20X_OCV(i),
						 0xff, ocv[i]);
			if (ret)
				dev_warn(&pdev->dev,
					 "Failed to store OCV[%d] setting: %d\n",
					 i, ret);
		}
	regmap_update_bits(axp20x->regmap, AXP20X_FG_RES, AXP20X_FG_ENABLE, AXP20X_FG_ENABLE);

	psy_cfg.of_node = pdev->dev.of_node;
	psy_cfg.drv_data = power;

	power->supply = devm_power_supply_register(&pdev->dev, battery_power_desc,
						   &psy_cfg);
	if (IS_ERR(power->supply))
		return PTR_ERR(power->supply);

	/* Request irqs after registering, as irqs may trigger immediately */
	for (i = 0; irq_names[i]; i++) {
		irq = platform_get_irq_byname(pdev, irq_names[i]);
		if (irq < 0) {
			dev_warn(&pdev->dev, "No IRQ for %s: %d\n",
				 irq_names[i], irq);
			continue;
		}
		irq = regmap_irq_get_virq(axp20x->regmap_irqc, irq);
		ret = devm_request_any_context_irq(&pdev->dev, irq,
				axp20x_battery_power_irq, 0, DRVNAME, power);
		if (ret < 0)
			dev_warn(&pdev->dev, "Error requesting %s IRQ: %d\n",
				 irq_names[i], ret);
	}

	return 0;
}

static const struct of_device_id axp20x_battery_power_match[] = {
	{ .compatible = "x-powers,axp202-battery-power-supply" },
	{ .compatible = "x-powers,axp221-battery-power-supply" },
	{ }
};

MODULE_DEVICE_TABLE(of, axp20x_battery_power_match);

static struct platform_driver axp20x_battery_power_driver = {
	.probe = axp20x_battery_power_probe,
	.driver = {
		.name = DRVNAME,
		.of_match_table = axp20x_battery_power_match,
	},
};

module_platform_driver(axp20x_battery_power_driver);

MODULE_AUTHOR("Hans de Goede <hdegoede@redhat.com>");
MODULE_DESCRIPTION("AXP20x PMIC AC power supply status driver");
MODULE_LICENSE("GPL");
